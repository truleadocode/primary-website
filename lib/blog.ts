...
/**
 * Inline figure with an Unsplash image.
 * Inline styles on every element so images render even if .blog-content CSS
 * hasn't cascaded yet — belt AND suspenders.
 */
function fig(unsplashId: string, alt: string, caption: string): string {
  const src = `https://images.unsplash.com/${unsplashId}?w=960&h=480&auto=format&fit=crop&q=80`
  return [
    `<figure style="margin:2.5rem 0;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0;background:#f8fafc;display:block;">`,
    `<img src="${src}" alt="${alt}" width="960" height="480" style="width:100%;height:320px;object-fit:cover;display:block;max-width:100%;" />`,
    `<figcaption style="font-size:0.8125rem;color:#94a3b8;text-align:center;padding:0.75rem 1.25rem 0.875rem;font-style:italic;background:#f8fafc;border-top:1px solid #e2e8f0;display:block;">${caption}</figcaption>`,
    `</figure>`,
  ].join('\n')
}

/** Internal link helper with descriptive anchor text. */
...
export const blogPosts: BlogPost[] = [
  ...
  // =========================================================================
  {
    ...
    content: [
      ...
      fig('photo-1522202176988-66273c2fd55f', 'Marketing agency team collaborating on influencer campaign strategy', 'Influencer marketing software replaces spreadsheets and email chains with a single structured workspace for your entire team'),
      ...
      fig('photo-1504868584819-f8e8b4b6d7e3', 'Business analytics dashboard showing campaign performance metrics and ROI data', 'One-click reporting turns raw campaign data into polished, client-ready decks — no manual spreadsheet work required'),
      ...
      `<p>A CRM tracks relationships. A spreadsheet tracks data. Influencer marketing software does both — and adds the campaign execution layer that neither can provide: structured approval workflows with audit trails, real-time campaign status across multiple clients, creator performance history, integrated payment processing, client-facing portals, and pre-built analytics templates.</p>`,
  ...
  // =========================================================================
  {
    ...
    content: [
      ...
      fig('photo-1600880292203-757bb62b4baf', 'Agency team overwhelmed with multiple simultaneous client campaigns and workstreams', 'Without the right systems in place, multi-client campaign management quickly becomes unsustainable — even for experienced teams'),
      ...
      fig('photo-1522071820081-009f0129c71c', 'Marketing team collaborating on campaign approval workflows and structured processes', 'Approval workflows built before launch eliminate the frantic email chains that slow down most agencies'),
  ...
  // =========================================================================
  {
    ...
    content: [
      ...
      fig('photo-1563986768609-322da13575f3', 'Business analyst reviewing influencer campaign ROI metrics and performance data on dashboard', 'Track ROI at the creator level — not just the campaign level — to know exactly which partnerships are driving results'),
      ...
      fig('photo-1549923746-c502d488b3ea', 'Marketing team setting up UTM tracking links and conversion attribution before campaign launch', 'Attribution infrastructure must be set up before launch — retroactive tracking always produces incomplete, unreliable data'),
  ...
  // =========================================================================
  {
    ...
    content: [
      ...
      `<p>Most influencer briefs fail for one of three reasons: they are too long and overwhelming, too vague and open-ended, or they read like a legal document rather than a creative guide. The most common failure mode is burying the most important information — the campaign objective and the one thing the creator absolutely must communicate — somewhere in the middle of a five-page PDF.</p>`,
      fig('photo-1455390582262-044cdead277a', 'Person writing notes in a notebook planning a creative brief for an influencer campaign', 'The best briefs are written from the creator perspective: what do they need to know to do their best work?'),
      ...
      `<p>List the non-negotiables — elements that must appear in the content. Keep this list short. Typical mandatory inclusions are: product name, key claim, promo code or link, and any required disclosures (e.g., #ad or #sponsored).</p>`,
      ...
      fig('photo-1542744173-8e7e53415bb0', 'Creative team reviewing campaign brand guidelines and influencer brief in a meeting', 'Aligning on brand voice before briefing creators prevents the most common reshoot scenario — content that looks nothing like your brand'),
      ...
      `<p>State the content draft deadline, the review window, the approval deadline, and the go-live date. If there is a required posting window (e.g., must go live between 9am–12pm on a Tuesday), state it explicitly.</p>`,
    ...
    faqs: [
      ...
      { question: 'Why do influencer campaigns need a brief?', answer: 'Without a brief, creators make assumptions about format, tone, key messages, and mandatory disclosures — leading to content that misses the mark, compliance issues, and costly reshoot rounds.' },
    ],
  },
  ...
  // =========================================================================
  {
    ...
    content: [
      ...
      fig('photo-1579621970563-ebec7560ff3e', 'Financial analyst reviewing earned media value calculations and marketing ROI metrics', 'EMV converts raw impressions into a dollar figure clients can benchmark directly against their paid media spend'),
      ...
      fig('photo-1554224155-6726b3ff858f', 'Business team reviewing campaign performance comparing earned media value against paid media ROI', 'Use EMV for awareness campaigns and ROI for conversion campaigns — the two metrics answer different questions'),
